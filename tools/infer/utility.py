...
def init_args():
    ...
    parser.add_argument("--video", type=str) # add new argument
    ...
    parser.add_argument(
        "--video_save_path", type=str, default="./inference_results/video") # add new argument
...
def get_object_region_and_mosaic(img, box, txt):
    """
    获得txt中的"aws"字符串对应的区域interest_box并对该区域进行mosaic
    :param img:
    :param box:
    :param txt:
    """
    image = img.copy()
    flag = False  # 是否含有"aws"字符串的标志
    len_txt = len(txt)
    lower_txt = txt.lower()
    interest_box = []  # 一张图中可能会有多个"aws"字符串
    if "aws" in lower_txt:
        flag = True
        idx_st = lower_txt.find("aws")  # "aws"开始的索引值
        idx_ed = idx_st + 2  # "aws"结束的索引值
        x0_split_ratio = (idx_st + 1) / len_txt
        x1_split_ratio = (idx_ed + 1) / len_txt
        x0_split = (box[1][0] - box[0][0]) * x0_split_ratio  # 左上角的偏移量
        x1_split = (box[1][0] - box[0][0]) * x1_split_ratio  # 右上角的偏移量

        upper_left = (int(box[0][0] + x0_split), box[0][1])
        upper_right = (int(box[0][0] + x1_split), box[1][1])
        lower_right = (int(box[3][0] + x1_split), box[2][1])
        lower_left = (int(box[3][0] + x0_split), box[3][1])
        interest_box = [upper_left, upper_right, lower_right, lower_left]
...
def get_object_region(box, txt):
    """
    获得txt中的"aws"字符串对应的区域interest_box
    : param box:
    : param txt:
    """
    flag = False # 是否含有"aws"字符串的标志
    ...
    if "aws" in lower_txt:
        ...
        lower_right = (int(box[3][0] + x1_split), box[2][1])
        lower_left = (int(box[3][0] + x0_split), box[3][1])
        single_interest_box = [upper_left, upper_right, lower_right, lower_left]
        interest_box.append(single_interest_box)

    if flag:
        return interest_box, flag
    else:
        return box, flag


def put_mosaic(image, box):
    """
    根据opencv打开过的image对感兴趣区域box进行马赛克
    补充：box格式 [upper_left, upper_right, lower_right, lower_left]
    :param image:
    :param box
    return
    """
    ...
    for single_box in box:
        for m in range(single_box[0][1], single_box[3][1]-step):
            for n in range(single_box[0][0], single_box[1][0]-step):
                for i in range(step):
                    for j in range(step):
                        b,g,r = image[m, n]
                        image[m+i, n+j] = (b, g, r)
...
if __name__ == '__main__':
    box = [[97, 1079], [1677, 1050], [1680, 1165], [99, 1194]]
    txt = "warp speed with Aws"

    # box = [[898, 1256], [1610, 1256], [1610, 1309], [898, 1309]]
    # txt = "Sundar Shankar (he/him)"
    res = get_object_region(box, txt)
    print(res)
